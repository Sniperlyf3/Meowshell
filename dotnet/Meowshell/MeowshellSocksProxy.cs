        ...
        MeowshellSocksOptions options, Action<string>? onLog = null)
    {
        ...
        try
        {
            ...
            process.Exited += async (_, _) =>
            {
                ...
                if (proxy._stopped) proxy._exited.TrySetResult();
                else proxy._exited.TrySetException(new TailcatException(
                    "tailcat exited unexpectedly", process.ExitCode, proxy._diagnostics.Tail()));
            };
            process.OutputDataReceived += (_, e) => { if (e.Data is not null) { proxy.Log?.Invoke(e.Data); onLog?.Invoke(e.Data); } };
            process.ErrorDataReceived += (_, e) =>
            {
                if (e.Data is null) return;
                proxy._diagnostics.Add(e.Data);
                proxy.Log?.Invoke(e.Data);
                onLog?.Invoke(e.Data);
            };
